...
SMRMotion loadMotionFromVSK(string _skelFileName, string _motionfilename)
{
  ...
  xmlTextReaderPtr reader;
  reader = xmlReaderForFile(_skelFileName.c_str(), NULL, 0);
  map<string,double> parameters = SmrLoaderVskStoreVskParameters(reader);
    ...
    if ( !strcmp(buffer,"DATAGROUP") )
    {
      ...
      while(parsedSectionChars < sectionLength)
      {
        ...
        }else
        {
          ...
          myMotion.setTimeStep(1.0 / frameRate );
        }
    ...
    }else
    {
      ...
      for (int i=0; i < (numBodies / 6); i++)
      {
        ...
        if (i < (newPose.getNumJoints()))
        {
          ...
          {
#endif
            //SMRVector3 localPos = newPose.getJoint(i)->getPosition();
      ...
      myMotion.insertSkeleton(newPose);
    }
    file.read(buffer,2);
