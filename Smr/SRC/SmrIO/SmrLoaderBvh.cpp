...
{
    stringbuf _contentStringBuf(_content);
    istream contentStream(&_contentStringBuf);
    return loadMotionFromStream(contentStream,_type);
}
...
SMRMotion loadMotionFromBVH(string _filename, SMRTransformationOrderType _type)
{
    ...
    SMRMotion motion = loadMotionFromStream(infile, _type);
    infile.close();
    return motion;
}
...
SMRMotion loadMotionFromStream(istream &_contentStream, SMRTransformationOrderType _type)
{
    ...
    expect("HIERARCHY", _contentStream);
    expect("ROOT", _contentStream);
    ...
    string nextToken("");
    _contentStream >> nextToken;
    string nodeName("");
    do
    {
        nodeName = nextToken;
        
        expect("{", _contentStream);
        expect("OFFSET", _contentStream);
        
        double offsetX,offsetY,offsetZ;
        _contentStream >> offsetX; _contentStream >> offsetY; _contentStream >> offsetZ;
        
        expect("CHANNELS", _contentStream);
        
        int nDofs = 0;
        _contentStream >> nDofs;
        ...
        if(nextToken == "End")
        {
            ...
            _contentStream >> endOffsetX;
            _contentStream >> endOffsetY;
            _contentStream >> endOffsetZ;
    ...
    expect("Frames:", _contentStream);
    unsigned int numFrames;
    _contentStream >> numFrames;
    //#ifdef TRACE
    cout << "Number of frames in motion: " << numFrames << endl;
    //#endif
    
    expect("Frame", _contentStream);
    expect("Time:", _contentStream);
    //string nextToken;
    double alpha;
    _contentStream >> alpha;
